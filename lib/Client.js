...
class Client extends EventEmitter {
    ...
    */
    createGuildEmoji(guildID, options, reason) {
        options.reason = reason;
        return this.requestHandler.request("POST", Endpoints.GUILD_EMOJIS(guildID), true, options).then((emoji) => new Emoji(emoji, this.guilds.get(guildID)));
    }
    ...
    */
    editGuildEmoji(guildID, emojiID, options, reason) {
        options.reason = reason;
        return this.requestHandler.request("PATCH", Endpoints.GUILD_EMOJI(guildID, emojiID), true, options).then((emoji) => new Emoji(emoji, this.guilds.get(guildID)));
    }
    ...
    */
    getRESTGuildEmoji(guildID, emojiID) {
        ...
        return this.requestHandler.request("GET", Endpoints.GUILD_EMOJI(guildID, emojiID), true).then((emoji) => new Emoji(emoji, null));
    }

    /**
    * Get a guild's emojis via the REST API. REST mode is required to use this endpoint.
    * @arg {String} guildID The ID of the guild
    * @returns {Promise<Array<Emoji>>}
    */
    getRESTGuildEmojis(guildID) {
        if(!this.options.restMode) {
            return Promise.reject(new Error("Eris REST mode is not enabled"));
        }
        return this.requestHandler.request("GET", Endpoints.GUILD_EMOJIS(guildID), true).then((emojis) => emojis.map((emoji) => new Emoji(emoji, null)));
    }
